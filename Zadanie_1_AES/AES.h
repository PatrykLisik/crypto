//
// Created by bambi on 10.10.18.
//

#ifndef AES_AES_H
#define AES_AES_H

#include <iostream>


class AES {
private:
    int word;       //Number of bytes in column
    int N;          //Length of the key
    int round;      //Round of ciphering
public:
    AES(int word, int N, int round);

private:
    void KeyExpansion(unsigned char* key, unsigned char* keysForAllRounds);
    void RotWord(unsigned char* word);
    void SubWord(unsigned char *word);
    void RoundConstant(unsigned char *rcon, int currentRound);
    void XORWord(unsigned char *copy, unsigned char *column);

    void SubBytes(unsigned char **state);
    void ShiftRows(unsigned char **state);
    void MixColumns(unsigned char **state);
    void AddRoundKey(unsigned char **state, unsigned char* roundKey);

public:
    void Encrypt(unsigned char* plainText, unsigned char* key);
    void Decrypt();
    void GenerateKey();


    void setCurrentRoundKey(unsigned char *currentRoundKey, unsigned char *keyForAllRoundsm, int currentRound);

    void ShiftRow(unsigned char **state, int row, int fieldsNumber);

    void MultiplyGaloisFieldByWord(unsigned char *word);

    void printState(unsigned char **state);

    unsigned char MultiplyModulo(const unsigned char field, const unsigned char val);

    void RotateState(unsigned char **state);
};

const unsigned char SBox[16][16] = {
        
};

const unsigned char GaloisField[4][4] = {
        0x02, 0x03, 0x01, 0x01,
        0x01, 0x02, 0x03, 0x01,
        0x01, 0x01, 0x02, 0x03,
        0x03, 0x01, 0x01, 0x02
};


#endif //AES_AES_H
